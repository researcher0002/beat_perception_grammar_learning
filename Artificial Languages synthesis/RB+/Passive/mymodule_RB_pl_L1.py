...
def get_time(X,dictionary,syll):
    #X= the structure with the sequence, Dictionary= the dictionary and syll= the syllable I want to know the onset time of
    T=[]
    for row in range(len(X)):
        seq= X[row]
        c=0
...
def create_sound(X):
    #structure with the single sequence with the single syllables and silences
