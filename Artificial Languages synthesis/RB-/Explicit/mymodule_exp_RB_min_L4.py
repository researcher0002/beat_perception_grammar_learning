...
inpPath = os.getcwd()+'\\'
outPath=inpPath+'testRB_min_L4\\'
NADpath = 'NADsil/'
xsyllpath='xsyll/'

#1 xayll used in the functions 
xsyllpath='xsyll//'
...
def structure(times, rule, position, D):
    ...
    for i in range(times):
        if len(rule)==2:
            ...
            chunk.insert(4, AudioSegment.silent(duration=239))
            chunk.insert(8, AudioSegment.silent(duration=238))
